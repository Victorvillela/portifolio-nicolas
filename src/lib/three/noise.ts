/*
  Simplex noise 3D (CPU) — usado apenas na geração inicial de posições,
  nunca por frame. Implementação clássica de Stefan Gustavson, compacta.
*/
...
function buildPermutation(seed: number): Uint8Array {
  ...
  let s = seed >>> 0;
  const rand = () => {
    // xorshift32 — determinístico por seed
    s ^= s << 13;
    s ^= s >>> 17;
    s ^= s << 5;
...
/* Gerador pseudo-aleatório determinístico (mulberry32) para reprodutibilidade */
